...
namespace HandCarftBaseServer.Controllers
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        /// <summary>
        /// دریافت لیست دسترسی های گروه کاربری  
        /// </summary>
        [Authorize]
        [HttpGet]
        [Route("Role/GetRoleAccessList")]
        public IActionResult GetRoleAccessList(long roleId)
        {
            try
            {

                var res = _repository.RoleForms.FindByCondition(c => c.Ddate == null && c.DaDate == null && c.RoleId == roleId)
                    .Include(c => c.Forms)
                    .Select(c => new { RoleFormId = c.Id, c.FormsId, FormName = c.Forms.Name, c.DeleteRule, c.InsertRule, c.ViewRule, c.UpdateRule })
                    .ToList();
        ...
        /// <summary>
        /// ثبت دسترسی های گروه کاربری  
        /// </summary>
        [Authorize]
        [HttpPost]
        [Route("Role/InsertRoleAccess")]
        public IActionResult InsertRoleAccess(RoleAccessInsertDto input)
        {
            try
            {
                ...
                if (input.FormList.Count == 0)
                    throw new BusinessException(XError.BusinessErrors.FormNotSelected());

                #endregion

                var tobedeleted = _repository.RoleForms.FindByCondition(c => c.RoleId == input.RoleId).ToList();
                ...
                _repository.RoleForms.UpdateRange(tobedeleted);

                input.FormList.ForEach(c =>
                {
                    var roleform = new RoleForms
                    {
                        Cdate = DateTime.Now.Ticks,
                        CuserId = userId,
                        DeleteRule = c.DeleteRule,
                        FormsId = c.FormsId,
                        InsertRule = c.InsertRule,
                        RoleId = input.RoleId,
                        UpdateRule = c.UpdateRule
                    };
                    _repository.RoleForms.Create(roleform);
                });
